...
extension StationDatabaseExtensions on DatabaseService {
  ...
  Future<List<Station>> getEligibleStations({
    ...
    final maps = await db.query(
      ...
      where:
          'evacuationCenterId = ? '
          'AND (allowedAgeGroup IS NULL OR allowedAgeGroup = ? OR allowedAgeGroup = ?) '
          'AND (allowedMedicalCondition IS NULL OR allowedMedicalCondition = ? OR allowedMedicalCondition = ?) '
          'AND active = 1',
      whereArgs: [
        centerId,
        ageGroup.index,
